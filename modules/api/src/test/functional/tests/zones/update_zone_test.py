...
def test_update_zone_with_non_ldap_email_fails(shared_zone_test_context):
    """
    Test that updating a zone with an email which doesn't exist in ldap fails
    """
    client = shared_zone_test_context.ok_vinyldns_client
    zone = copy.deepcopy(shared_zone_test_context.ok_zone)

    zone["email"] = "trial@dummy.com"

    error = client.update_zone(zone, status=400)
